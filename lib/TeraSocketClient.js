...
class TeraSocketClient extends EventEmitter {
    ...
    parseArrayType(field, payload, parsed, opcodeName) {
        ...
        let expectedHere = refOffset;
        for (let k = 0; k < count; k++) {
            if (currentOffset + 4 > payload.length) {
                this.onWarning(`Array overrun at ${currentOffset} for ${field.name} (item ${k}/${count})`);
                this.writeToLogs('parse_errors.log', `Array overrun at ${currentOffset} for ${field.name} (item ${k}/${count}) in ${opcodeName}\n`);
                break;
            }

            const here = payload.readUInt16LE(currentOffset);
            const next = payload.readUInt16LE(currentOffset + 2);
            currentOffset += 4;  // FIXED: Skip header for all items

            if (here !== expectedHere) {
                this.onWarning(`Here mismatch: ${here} != expected ${expectedHere} for item ${k} in ${field.name}`);
                this.writeToLogs('parse_errors.log', `Here mismatch: ${here} != expected ${expectedHere} for item ${k} in ${field.name} in ${opcodeName}\n`);
            }

            const item = {};
            let validItem = true;
            let maxOffset = currentOffset;  // Track farthest position for this item
            for (const subField of field.subType || []) {
                if (currentOffset >= payload.length) {
                    this.onWarning(`Array item overrun at ${currentOffset} for ${subField.name} in item ${k} of ${opcodeName}`);
                    this.writeToLogs('parse_errors.log', `Array item overrun at ${currentOffset} for ${subField.name} in item ${k} of ${opcodeName}\n`);
                    validItem = false;
                    break;
                }
                
                let parseOffset = currentOffset;

                const {
                    value,
                    length
                } = parsePayloadByType(subField.type, payload, parseOffset, null, opcodeName, subField.name);

                if (length === 0) {
                    if (subField.type !== 'string') {
                        this.onWarning(`Zero length for fixed subfield ${subField.name} in item ${k} of ${opcodeName} at ${parseOffset}`);
                        this.writeToLogs('parse_errors.log', `Zero length for fixed subfield ${subField.name} in item ${k} of ${opcodeName} at ${parseOffset}\n`);
                        validItem = false;
                        break;
                    } else {
                        this.onWarning(`Zero length string for ${subField.name} in item ${k} of ${opcodeName} at ${parseOffset} (empty?)`);
                        this.writeToLogs('parse_errors.log', `Zero length string for ${subField.name} in item ${k} of ${opcodeName} at ${parseOffset}\n`);
                        item[subField.name] = '';
                        // No advance
                        continue;
                    }
                ...
                maxOffset = Math.max(maxOffset, parseOffset + length);  // FIXED: Cover jumped strings
            }

            // FIXED: Advance to max (covers strings) but respect next chain for fixed
            currentOffset = Math.max(currentOffset, maxOffset);

            if (validItem) {
                items.push(item);
            } else {
                this.onWarning(`Invalid item ${k} in ${field.name}; stopping array parse`);
                break;
            }
            ...
            if (next === 0) {
                if (k < count - 1) {
                    this.onWarning(`Early array end at item ${k + 1}/${count} in ${opcodeName}`);
                    this.writeToLogs('parse_errors.log', `Early array end at item ${k + 1}/${count} in ${opcodeName}\n`);
                    break;
                }
            } else {
                currentOffset = next;  // Jump to next fixed start (resets over string advances)
            }
        }
        ...
        if (definition) {
            ...
            for (const field of definition) {
                ...
                if (field.type === 'array' || field.type === 'object') {
                    ...
                    buffer_pos = refOffset - 4; // adjust for header
                    const {
                        value,
                        length
                    } = parsePayloadByType(field.type, payload, buffer_pos, null, opcodeName, field.name);
                    parsed[field.name] = value;
                    // don't advance main offset, strings are jumped to via ref
