            ...
            if (definition) {
                ...
                for (const field of definition) {
                    if (offset >= payload.length) {
                        this.onWarning(`Parse overrun at field ${field.name} for ${opcodeName} at offset ${offset} (payload len: ${payload.length})`);
                        this.writeToLogs('parse_errors.log', `Overrun in ${opcodeName} at field ${field.name}, offset ${offset}, payload length ${payload.length}\n`);
                        break;
                    }
                    ...
                    if (field.type === 'array' || field.type === 'object') {
                        ...
                        if (length === 0) {
                            this.onWarning(`Zero length for field ${field.name} in ${opcodeName} at ${offset}`);
                            this.writeToLogs('parse_errors.log', `Zero length for field ${field.name} in ${opcodeName} at ${offset}\n`);
                            break;
                        }
                        parsed[field.name] = value;
                        offset += length;
                ...
                if (offset !== payload.length) {
                    this.onWarning(`Partial parse for ${opcodeName}: used ${offset}/${payload.length} bytes`);
                    this.writeToLogs('parse_errors.log', `Partial parse for ${opcodeName}: used ${offset}/${payload.length} bytes\n`);
                }
