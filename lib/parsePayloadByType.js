    ...
    switch (type) {
        ...
        case 'byte': {
            ...
            if (offset + 1 >= payload.length) {
                ...
                fs.appendFileSync('error.log', `byte length read overrun at ${offset} for ${fieldName} in ${opcodeName}\n`);
            ...
            if (len > maxLen) {
                ...
                fs.appendFileSync('error.log', `Excessive byte length ${len} at ${offset} for ${fieldName} in ${opcodeName}, capping at ${maxLen}\n`);
            ...
            if (end > payload.length) {
                ...
                fs.appendFileSync('error.log', `byte data overrun at ${offset}, len ${len} for ${fieldName} in ${opcodeName}\n`);
                return {value: Buffer.alloc(0), length: 2};
            }
